...
def plot_3Dsurface(x: np.ndarray, y: np.ndarray, z: np.ndarray, size=(20, 10), 
                save_path_2d=None, save_path_3d=None, cmap='rainbow', xlabel: str='', ylabel: str='', zlabel: str=''):
    fig = plt.figure(figsize=size)
    ax = fig.add_subplot(111, projection='3d')
    X,Y = np.meshgrid(x,y)
    mycmap = plt.get_cmap(cmap)
    surf = ax.plot_surface(X=X, Y=Y, Z=z, cmap=mycmap)
    fig.colorbar(surf, ax=ax, shrink=1, aspect=3)
    ax.set_xlabel(xlabel); ax.set_ylabel(ylabel); ax.set_zlabel(zlabel)
    # ax.legend()
    if save_path_3d is not None:
        plt.savefig(save_path_3d, dpi=200)
    # plt.show()
    if save_path_2d is not None:
        ax.view_init(elev=90, azim=-90)
        plt.savefig(save_path_2d, dpi=200)
    plt.clf()
    
# def plot_3DsurfaceProj(x: np.ndarray, y: np.ndarray, z: np.ndarray, 
#                 size=(20, 10), save_path=None, xlabel: str='', ylabel: str='', zlabel: str=''):
