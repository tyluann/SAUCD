...
def CurvatureCwF(vertices,faces,opt=1,want_LB=False):
  ...
  for i in range(npt):
    ...
    # 2-ring
      ...
      nbrs=np.unique(np.hstack(neighbor_verts[neighbor_verts[i]]))
    ...
    for j,pj in enumerate(vertices[nbrs]):
      ...
      scale=1.0/(np.sqrt(u**2+v**2) + 1e-10)
    ...
    VN[i]/=np.sqrt(VN[i,0]**2+VN[i,1]**2+VN[i,2]**2) + 1e-10
    ...
    H[i]  = (E*N+G*L-2*F*M)/(2*(E*G-F**2) + 1e-10)
    K[i]  = (L*N-M**2)/(E*G-F**2 + 1e-10)
  ...
  for i in range(npt):
    ...
    for j in range(len(nbrs)):
      k=np.mod(j+1,len(nbrs))
      v1=vertices[i]-vertices[nbrs[k]]
      v2=vertices[i]-vertices[nbrs[j]]
      S+=np.linalg.norm(fastcross(v1, v2))
    dS[i]=S/6
  ...
  #Laplace Beltrami from fitting
    ...
    for i in range(npt):
      ...
      if(opt==2 or len(neighbor_verts[i])<5 ):
      # 2-ring
        nbrs=np.unique(np.hstack(neighbor_verts[neighbor_verts[i]].flatten()))
        nbrs=np.setdiff1d(nbrs,i)
      else:
        nbrs=neighbor_verts[i]
      ...
      hu=X4[0]
      hv=X4[1]
      if metric[i,0,0]==0 and metric[i,1,1]==0 and metric[i,0,1]==0 and metric[i,1,0]==0:
        LB[i]=0
      else:
